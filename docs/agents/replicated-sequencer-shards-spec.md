...
> `sequencer.nomad.hcl` is expanded by Nomad HCL
> from `config/shard-map.toml`: one group `seq-<lane>` per active lane,
> `count = 2` with `distinct_hosts`, an explicit `--lane` and `--vslots`, and
> a metrics port lane (`9001 + 10 * lane`) with a Nomad dynamic port for
> the cluster egress. Two lanes share a node without a port clash, and
