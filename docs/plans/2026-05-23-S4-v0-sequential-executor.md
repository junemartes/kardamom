...
> **⚠️ ARCHITECTURE UPDATE — 2026-05-24 (D-Sh12):** the executor no longer subscribes to a single channel B carrying full `TxEnvelope`s. It now subscribes to **M channel A's + 1 channel B**: A-readers buffer `TxEnvelope`s in an in-RAM map keyed by `(sequencer_id, position_a)`; B-reader walks the canonical orderer and for each `TxRef` joins to the buffered envelope from the corresponding A. Execution still happens in canonical B-position order; B-position remains `tx_idx`. The state-snapshot swap, Block-STM deferrals, and channel-C emission are unchanged. See `docs/plans/2026-05-23-S0-shared-decisions.md` D-Sh12 and `docs/specs/2026-05-23-high-throughput-sequencer-design.md` D11 / §2.4 for the full model. Reader-thread and join-buffer tasks below need to be reshaped at implementation time.
