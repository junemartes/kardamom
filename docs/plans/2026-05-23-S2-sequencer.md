...
- **No `kardamom_log::framing::TxFrame`.** TxOrdering carries `TxEnvelope` directly, rkyv-archived via `kardamom_log::codec::{encode, access, materialize}`. The sequencer publishes the `TxEnvelope` byte payload onto B; downstream consumers do `kardamom_log::codec::access::<TxEnvelope>(bytes)` for zero-copy reads.
