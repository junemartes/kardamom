...
> **⚠️ ARCHITECTURE UPDATE — 2026-05-24 (D-Sh12):** the sequencer no longer publishes the full `TxEnvelope` directly to channel B. Instead it does a **dual write**: full `TxEnvelope` → its own **channel A[i]** (Aeron *exclusive* publisher, persisted), then a tiny `TxRef { sequencer_id, position_a }` → channel B (Aeron *concurrent* multi-publisher, also persisted; canonical orderer). `pending_nonce.advance()` is gated on both writes completing. See `docs/plans/2026-05-23-S0-shared-decisions.md` D-Sh12 and `docs/specs/2026-05-23-high-throughput-sequencer-design.md` D11 / §2.2 for the full model. The publish step in Task 9 / Task 11 below predates this change — rewrite to dual-write at implementation time.
