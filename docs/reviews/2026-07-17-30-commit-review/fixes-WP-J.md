...
## F07.3 [M] ~~FIXED~~ REVERTED — replay served synchronously on the cluster service thread (1s/frame deadline)
> **REVERTED post-CI**: the timer-driven chunked drain (and its companion
> mid-replay live-broadcast skip) changed the steady-state egress flow and
> correlates with the all-shards cluster-e2e consumer freeze at first-record
> time; replay is served synchronously again, exactly as on main. See
> fixes-CI-replay-loop.md. The original text below is kept for the record.
