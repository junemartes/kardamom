...
`chaos-ingress`, `chaos-sequencer`, `chaos-cluster`, `chaos-fleet`, `chaos-retention`,
`chaos-cache`). A
...
| `chaos-fleet` | every replica of one role down at once: **cluster-quorum-loss-recover** (2 of 3 sealers) / **cluster-total-loss-recover** (all 3 sealers) / **executor-fleet-loss-recover** (all 3 executor nodes) / **executor-fleet-wipe-recover** (all 3 executor nodes, state DBs wiped, local checkpoints kept); the load keeps submitting after the fleet returns |
