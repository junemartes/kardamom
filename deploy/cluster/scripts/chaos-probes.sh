...
seqa_metric() { # <metric-name> -> integer sum across label lines (empty on scrape failure)
  # Lane 0's replica on node-0 exposes metrics on :9001 (9001 + 10 * lane).
  # This tries the bridge address first, then falls back to docker exec,
  # mirroring assert_replica_healthy's probe.
  local body
  body="$(fetch_metrics "${NODE_IP[kardamom-sequencer-0]}" kardamom-sequencer-0 9001 || true)"
  prom_value "${body}" "$1" sum
}
seqb_twin_metric() { # <metric-name> — lane 0's other replica, on node-1 (:9001)
  local body
  body="$(fetch_metrics "${NODE_IP[kardamom-sequencer-1]}" kardamom-sequencer-1 9001 || true)"
  prom_value "${body}" "$1" sum
}
