...
// ---------------------------------------------------------------------------
// Async log handles to sync engine traits, with the per-shard pump tasks.
// ---------------------------------------------------------------------------

/// The live tx_data subscription both role binaries run. It is one Aeron
/// subscriber handle per shard, bridged from async to sync through a pump
/// task. This is public so a binary's `EngineWiring` can name it as its
/// `TxData` type.
...
/// Open the M per-shard tx_data subscriptions. This bridges each handle's
/// async `recv()` to the synchronous `next()` the engine's reader threads
/// expect, with a dedicated tokio pump task per shard. Call this inside a
/// tokio runtime.
///
