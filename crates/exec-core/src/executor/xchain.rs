...
use kardamom_types::{BPosition, Receipt, SkipReason, StateDatabase};
...
/// (`failed_xchain_receipt`). Only local failures (database errors), or a
/// `gas_limit` above the block gas limit, are engine errors.
///
/// # Errors
///
/// Returns [`ExecutorError::Execution`] when `message.gas_limit` is above
/// the block gas limit, the inner call fails non-deterministically, or a
/// database read fails.
pub fn execute_xchain_tx<S: StateDatabase>(
    ...
    if let Some((reason, detail)) = xchain_value_rejection(origin_chain_id, delivery.message) {
