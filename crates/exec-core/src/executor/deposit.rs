...
pub fn execute_deposit_tx<S: StateDatabase>(
    ...
    // (1) Mint pre-credit, committed unconditionally: the mint is durable
    // no matter the inner-call outcome.
    call.credit_mint(deposit)?;

    // (2) Inner EVM call. Disable the nonce check, since deposits do not
    // carry a nonce.
    let outcome = call.transact_commit(tx_env_from_deposit(deposit))?;
...
mod tests {
    ...
    fn deposit_mints_and_forwards_value() {
        ...
        let (receipt, ws) =
            execute_deposit_tx(&snap, None, &delta, env, slot(0, 0, 0, 0), &d, None)
                .expect("execute");
    ...
    fn deposit_mint_overflow_returns_execution_error() {
        ...
        let err =
            execute_deposit_tx(&snap, None, &delta, env, slot(0, 0, 0, 0), &d, None).unwrap_err();
        assert!(
    ...
    /// THE EQUIVALENCE GATE for the on-scope deposit path: the
    /// fresh-cache path (`execute_deposit_tx`) and `Executor::execute_deposit`
    /// must produce byte-identical artifacts — receipt (`write_set_hash`
    ...
    fn old_and_new_deposit_paths_agree() {
        ...
        for (name, d) in cases {
            ...
            let (new_r, new_ws) = scope
                .execute_deposit(slot(0, 0, 0, 0), &d, Some((&mut new_bal, 1)))
