...
fn xchain_delivery_produces_a_0x7d_receipt_keyed_by_source_hash() {
    ...
    assert_eq!(ws.account(&sender).expect("caller touched").nonce, 1);
...
/// alone is far above `XCHAIN_DELIVERY_OVERHEAD`. The budget covers it,
/// so the delivery executes: `Ok`, and not a skip marker.
#[test]
fn max_payload_with_zero_gas_limit_delivers() {
    ...
    for len in [MAX_DATA_BYTES, 3_200] {
        ...
        m.gas_limit = 0;
        let (receipt, _ws) =
            run_free(&snap, &delta, &m, 0, None).unwrap_or_else(|e| panic!("{len} bytes: {e:?}"));
        assert!(
...
/// against the same prior layer and message, the shared-artifact
/// check, and the case's own shape check, so a silently degenerate
/// case cannot pass.
fn run_xchain_case(
    name: &str,
    m: &XChainMessage,
    ...
    assert_xchain_case_shape(name, &old_r, &old_ws, m);
}

/// The per-case shape check `run_xchain_case` runs after the shared
/// artifacts agree: which fields matter differs by case name, so a
/// silently degenerate case (an empty write set, no status) cannot
/// pass just because the two paths agree on it.
fn assert_xchain_case_shape(name: &str, old_r: &Receipt, old_ws: &WriteSet, m: &XChainMessage) {
    match name {
        ...
        "target writes storage" => {
            assert!(old_r.status, "{name}");
            assert!(
                old_ws.storage.iter().any(|((a, _), _)| *a == m.target),
                "{name}"
            );
        }
        "duplicate delivery" => {
...
fn old_and_new_xchain_paths_agree() {
    ...
    for (name, m, deliver_first) in cases {
        run_xchain_case(name, &m, deliver_first, &snap, env, &base_prior);
    }
}
