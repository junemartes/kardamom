...
/// Time RLP decode for one tx shape: build 2000 raw encodings of `input`,
/// warm up, then time `REPS` decode passes and print the result.
fn decode_one_shape(label: &str, input: &Bytes) {
    const REPS: usize = 20;
    let raws: Vec<Bytes> = (0..2000u64)
        .map(|nonce| {
            ...
            let sig = alloy_primitives::Signature::new(U256::from(1u64), U256::from(2u64), false);
