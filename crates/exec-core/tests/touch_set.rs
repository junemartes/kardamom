//! `TouchSet` capture (footprint shadow): per-tx read attribution from
//! `outcome.state`. This covers
//! accounts loaded but never touched, and slots accessed without a value
//! change. The block-scoped BAL cannot see either per tx, which is why
//! the capture lives in `ExecScope::execute_tx` itself.
...
/// PUSH1 1, PUSH1 0, SSTORE, STOP. Writes slot 0 to 1.
...
/// account (the class the Accumulator runtime guard cares about).
...
fn touches_none_is_a_no_op() {
    ...
    // passes `None` and must behave the same way.
    let db = db();
    let mut scope = ExecScope::new(&db, None, env()).unwrap();
    let (receipt, _) = scope
        .execute_tx(
