...
struct RunArgs {
    ...
    #[arg(long, default_value_t = NonZeroU32::new(4000).expect("4000 != 0"))]
    ceiling: NonZeroU32,
    /// The ramp increment for each step, in tx/s.
    #[arg(long, default_value_t = NonZeroU32::new(250).expect("250 != 0"))]
    ramp_step: NonZeroU32,
    /// The number of seconds held at each ramp step.
    #[arg(long, default_value_t = NonZeroU64::new(12).expect("12 != 0"))]
...
fn load_cfg(a: &RunArgs, out: PathBuf) -> LoadConfig {
    LoadConfig {
        ...
        max_in_flight: NonZeroU32::new(1024).expect("1024 != 0"),
...
async fn run(a: RunArgs) -> anyhow::Result<()> {
    ...
    cfg.sender_range = SenderRange::new(1, NonZeroU32::new(15).expect("15 != 0"))?;
    ...
    cfg.sender_range = SenderRange::new(18, NonZeroU32::new(16).expect("16 != 0"))?;
    cfg.chaos_mode = true;
