...
use super::alloc::{AllocDelta, Bucket};
...
impl MdbxAbTotals {
    ...
    fn print_scheduling_detail(&self, wk: usize) {
        ...
        LBL.iter()
            .enumerate()
            .filter(|(i, _)| t.seq.buckets[*i].count + t.stm.buckets[*i].count > 0)
            .for_each(|(i, lbl)| {
                println!(
                    "       [{:>5}] seq {:>7.2}/tx {:>8.0}B | stm {:>7.2}/tx {:>8.0}B",
                    lbl,
                    avg_per_tx(t.seq.buckets[i].count, t.flow_txs),
                    avg_per_tx(t.seq.buckets[i].bytes, t.flow_txs),
                    avg_per_tx(t.stm.buckets[i].count, t.flow_txs),
                    avg_per_tx(t.stm.buckets[i].bytes, t.flow_txs),
                );
            });
        println!(
