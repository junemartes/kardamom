...
impl MdbxRun<'_> {
    ...
    fn run_block(
        ...
        let prepared: Vec<kardamom_stm::execute::PreparedTx> = recs
            .iter()
            .map(|(t, p, en)| kardamom_stm::execute::PreparedTx {
                tx_idx: *t,
                position: *p,
                envelope: en.clone(),
        ...
        let mut out =
            pool.run_block_prepared(views, PendingDelta::new(), e, prepared, &self.stats)?;
...
fn execute_sequential_side(
    ...
    let (seq_receipts, seq_delta) = if stm_only {
        ...
        let seq_txs: Vec<kardamom_stm::execute::SeqTx<'_>> = recs
            .iter()
            .zip(seq_decoded.iter())
            .map(|((t, p, en), d)| kardamom_stm::execute::SeqTx {
                tx_idx: *t,
                position: *p,
                envelope: en,
                decoded: d.as_ref(),
            ...
            .collect();
        kardamom_stm::execute::execute_block_sequential_decoded(snapshot, None, e, &seq_txs)?
    };
