...
use kardamom_stm::execute::{DeltaRelease, MvRelease, StmOutcome, execute_block_sequential};
...
impl Workload<'_> {
    ...
    fn settle_warmup(
        ...
        for BlockRecs { bi, recs } in warm_recs.iter().take(warm) {
...
fn all_recs(all_blocks: &[Vec<TxEnvelope>]) -> Vec<BlockRecs> {
    let mut gi = 0u64;
...
struct Settler<'a> {
    ...
    settle_rx: mpsc::Receiver<SettleJob>,
...
impl SettleCtx {
    ...
    fn settle_one(
        &self,
        pfi: usize,
        t: kardamom_stm::execute::BlockTicket,
    ) -> anyhow::Result<Option<RetainedOutcome>> {
        let out = t.wait()?;
        if self.timing {
            eprintln!(
...
impl Settler<'_> {
    ...
    fn spawn(self) -> SettlerHandle {
        ...
        std::thread::spawn(move || -> anyhow::Result<Vec<RetainedOutcome>> {
            ...
            while let Ok(SettleJob { pfi, ticket }) = settle_rx.recv() {
                outcomes.extend(ctx.settle_one(pfi, ticket)?);
            }
            Ok(outcomes)
        })
    }
...
impl PipelineRun<'_> {
    ...
    fn run_worker_count(&self, wk: std::num::NonZeroUsize) -> anyhow::Result<()> {
        /// The pipelined pool's fixed prune batch: at pipeline depth 2,
        /// small batches keep dispatch latency ahead of the next
        /// block's admission instead of trading it for graph-lock
        /// traffic (see `PoolConfig::prune_batch`).
        const PIPELINE_PRUNE_BATCH: std::num::NonZeroUsize =
        ...
        let cfg = self.eng.pool_config(wk, PIPELINE_PRUNE_BATCH, false);
        let wk = wk.get();
        ...
        kardamom_stm::execute::with_pool(cfg, |pool| -> anyhow::Result<()> {
            ...
            let (settle_tx, settle_rx) = mpsc::channel::<SettleJob>();
            let settler = Settler {
                writer_b: &writer_b,
                settle_rx,
                flow_recs: &flow_recs,
