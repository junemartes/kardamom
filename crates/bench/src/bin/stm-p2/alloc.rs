...
/// One size-class bucket's allocation count and byte total.
...
fn bucket_snap() -> [Bucket; 6] {
    std::array::from_fn(|i| Bucket {
        count: ALLOC_BUCKETS[i].load(std::sync::atomic::Ordering::Relaxed),
        bytes: BUCKET_BYTES[i].load(std::sync::atomic::Ordering::Relaxed),
    })
}
...
impl AllocDelta {
    ...
    pub(crate) fn since(before: &Self, after: &Self) -> Self {
        let mut buckets = [Bucket::default(); 6];
        buckets
            .iter_mut()
            .zip(before.buckets.iter().zip(after.buckets.iter()))
            .for_each(|(b, (bef, aft))| {
                *b = Bucket {
                    count: aft.count.saturating_sub(bef.count),
                    bytes: aft.bytes.saturating_sub(bef.bytes),
                };
            });
        Self {
