...
struct Args {
    ...
    #[arg(long, default_value_t = NonZeroU32::new(200).expect("200 != 0"))]
    target_tps: NonZeroU32,

    /// The number of sender accounts. Non-zero: `SignerSet::new`
    /// rejects an empty derived-signer set.
    #[arg(long, default_value_t = NonZeroU32::new(16).expect("16 != 0"))]
    senders: NonZeroU32,
    ...
    #[arg(long = "max-in-flight", default_value_t = NonZeroU32::new(256).expect("256 != 0"))]
    ...
    #[arg(long = "ramp-step-secs", default_value_t = NonZeroU64::new(15).expect("15 != 0"))]
...
async fn main() -> anyhow::Result<()> {
    ...
    // `--ramp-step-tps 0` means "auto": target_tps / 8, or 1 if that
    // floors to 0. This is the one real boundary; every downstream use
    // reads the resulting `NonZeroU32`.
    let ramp_step_tps = NonZeroU32::new(args.ramp_step_tps)
        .unwrap_or_else(|| NonZeroU32::new(args.target_tps.get() / 8).unwrap_or(NonZeroU32::MIN));
