...
/// Default `--duration` value used by the CLI when the user doesn't set
...
/// String form of [`DEFAULT_DURATION`] for `clap`'s `default_value`
/// (clap needs a `&str` that's then parsed by `humantime::parse_duration`).
pub const DEFAULT_DURATION_STR: &str = "10s";
