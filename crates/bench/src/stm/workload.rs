...
fn interleave_defi_block(
    ...
    while blk.len() < block_size {
        let ControlFlow::Continue(next_si) =
            interleave_step(queues, cursors, signers, block_size, si, &mut blk)
        else {
            break;
        };
        si = next_si;
...
/// One round-robin step of [`interleave_defi_block`]: push queue
/// `si`'s next transaction onto `blk`, if that queue still has one.
/// Returns the next `si` to try, or [`ControlFlow::Break`] once `si`
/// has looped past every queue twice over without filling `blk` (every
/// queue exhausted).
fn interleave_step(
    queues: &[Vec<crate::load::plan::PlannedTx>],
    cursors: &mut [usize],
    ...
    let next_si = si + 1;
    if next_si > block_size * queues.len() * 2 {
        return ControlFlow::Break(());
    }
    ControlFlow::Continue(next_si)
}
