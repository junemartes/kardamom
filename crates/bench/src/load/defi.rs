...
/// CLOB assigns IDs starting at 1), so a request for it is remapped to
/// ID 1: still a cheap no-op cancel, never an out-of-range one.
fn churn_id(seq: u64) -> U256 {
    U256::from(
        std::num::NonZeroU64::new(seq)
...
async fn confirm_deploy(
    ...
    loop {
        let ControlFlow::Continue(()) = confirm_tick(client, d, started, &mut state).await? else {
            return Ok(());
        };
...
/// One [`confirm_deploy`] poll: check for a landed receipt, else
