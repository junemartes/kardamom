...
use crate::load::engine::{
    Queues, RunHandles, SubmitMode, SubmitOpts, Tracker, drain, join_submit_tasks, pacer,
};
...
fn per_sender_estimate(cfg: &LoadConfig) -> usize {
    let ramp_steps = if cfg.chaos_mode || cfg.fixed_rate {
        ...
        u64::from(cfg.target_tps.get().div_ceil(cfg.ramp_step_tps.get()))
    };
    let total_secs = ramp_steps
        .saturating_mul(cfg.ramp_step_secs.get())
        .saturating_add(cfg.duration.as_secs());
    let est_total = u64::from(cfg.target_tps.get()).saturating_mul(total_secs);
    ((est_total as f64 * 1.2 / f64::from(cfg.sender_range.count().get())).ceil() as usize)
        .saturating_add(64)
}

/// [`build_queues`]'s result: the presigned per-sender submit queues,
/// plus any `DeFi` deployment transactions to land first.
struct BuiltQueues {
    queues: Queues,
    defi_deploys: Option<Vec<PlannedTx>>,
}

/// Build the presigned per-sender queues for `cfg.workload`, and the
/// `DeFi` deployment transactions to land first, if the workload needs them.
fn build_queues(
    cfg: &LoadConfig,
    signers: &SignerSet,
    chain_id: u64,
    ...
    let plan_params = TxPlanParams {
        chain_id,
        nonce_start: cfg.nonce_start,
        gas_price: cfg.gas_price,
    };
    let (queues_vec, defi_deploys) = match cfg.workload {
        Workload::Transfers => (
            plan::pregenerate(signers, cfg.to, cfg.value, per_sender, plan_params)?,
            None,
        ),
        Workload::Defi => {
...
/// The result of `spawn_receipt_feed`.
struct ReceiptFeed {
...
/// Start the receipt feed, if the mode needs it: subscribe mode always
/// needs it, and blocking mode needs it only when `feed_confirm`
/// replaces the per-transaction re-fetch. Returns whether the pacer
/// should trust the feed (`feed_confirm`) alongside its handle.
///
...
fn spawn_receipt_feed(
    cfg: &LoadConfig,
    signers: &[DerivedSigner],
    tracker: &Arc<Tracker>,
) -> ReceiptFeed {
    let confirm = cfg.feed_confirm_on();
    let task = if cfg.subscribe || confirm {
...
/// After the soak ends: join in-flight submit tasks so the tail is
/// classified and not left as merely "offered", drain the receipt
...
/// One run's config, handles, and tracker: the report-building and
/// ramp steps at the end of [`run`] are methods on this struct.
struct LoadRun<'a> {
    cfg: &'a LoadConfig,
    tracker: &'a Tracker,
}
...
async fn prepare_run(cfg: &LoadConfig) -> anyhow::Result<RunSetup> {
    ...
    let built = build_queues(cfg, &signers, chain_id, per_sender)?;
...
pub async fn run(cfg: LoadConfig) -> anyhow::Result<bool> {
    ...
    let ReceiptFeed {
        ...
        task: feed,
    } = spawn_receipt_feed(&cfg, &signers, &tracker);
    ...
    let discovered_max = if cfg.chaos_mode || cfg.fixed_rate {
        ...
        ramp_to_max(&cfg, &handles, &mut tasks, &scraper, &mut queues, &mut ramp).await
    };
    ...
    pacer(
        ...
        &mut tasks,
        &mut queues,
        // The clamp above guarantees at least 1; unwrap_or's fallback
        // never runs but keeps this call infallible rather than adding
        // a `# Panics` section to `run` for an unreachable case.
        std::num::NonZeroU32::new(soak_rate).unwrap_or(std::num::NonZeroU32::MIN),
        cfg.duration,
        SubmitOpts {
    ...
    let settled =
        settle_and_snapshot(&cfg, &client, &tracker, &mut tasks, &scraper, feed, sweeper).await;
    ...
    let report = run.build_report(verdict, ramp, discovered_max, soak_rate);
...
impl RampRun<'_> {
    ...
    async fn run_step(&mut self, rate: u32) -> RampStep {
        ...
        pacer(
            ...
            self.tasks,
            self.queues,
            // `rate` starts at `ramp_step_tps.get()` and only ever grows
            // by `saturating_add(ramp_step_tps.get())`, both at least 1;
            // unwrap_or's fallback never runs.
    ...
    async fn ramp_step(&mut self, rate: u32) -> Option<u32> {
...
async fn ramp_to_max(
    ...
    let mut run = RampRun {
        ...
        step_dur: Duration::from_secs(cfg.ramp_step_secs.get()),
    };
    let mut discovered = 0u32;
    let mut rate = cfg.ramp_step_tps.get();
    while rate <= cfg.target_tps.get() {
    ...
    discovered.max(cfg.ramp_step_tps.get())
}
