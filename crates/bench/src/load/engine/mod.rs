...
/// attempt, check for a receipt that already landed before the caller
/// retries.
async fn try_once(
    client: &HttpClient,
    tracker: &Tracker,
    ...
    if let Some(r) = receipt_status(client, tx.hash).await {
        tracker.confirm_with_gas(r.status, t0.elapsed(), r.gas);
...
/// Submit `tx`, retrying on error up to `retry` times.
async fn run_retries(
    ...
    for attempt in 0..=retry {
        let ControlFlow::Break(outcome) =
            try_once(client, tracker, tx, method, attempt, retry, t0).await
        else {
            continue;
        };
        return outcome;
...
async fn submit_task(
    ...
    let t0 = Instant::now();
    match run_retries(&client, &tracker, &tx, method, opts.retry, t0).await {
...
mod drain;
pub(crate) use drain::{drain, join_submit_tasks, spawn_pending_sweeper};
