...
//!
//! This lives in `kardamom-types`, not in the DA watcher, on purpose. Two
//! parties run the same rule for opposite reasons. The producer, the DA
//! watcher, emits an epoch's deposits into the canonical stream. The
//! verifier (the validator, and later the executor) re-derives them from
//! L1 and rejects a chain that disagrees. A verifier running a second,
//! subtly different copy of the rule would verify nothing. So there is
...
//!
//! The algorithm is OP-compatible and pinned by the contracts'
//! bytecode-hash CI gate, so this implementation must stay byte-identical.
...
fn source_hash_in_domain(domain: u64, l1_block_hash: B256, l1_log_index: u64) -> B256 {
    let inner = crate::rlp::encode_list_two(&l1_block_hash, &l1_log_index);
    ...
    let outer = crate::rlp::encode_list_two(&domain, &deposit_id_hash);
    keccak256(&outer)
}
...
pub fn alias_l1_address(l1: Address) -> Address {
    ...
    let a = crate::abi::word_address(l1);
    let b = crate::abi::word_address(Address::from(OFFSET));
...
fn deposit_from_lockbox_log(log: &LockboxLog) -> Deposit {
    match log {
...
/// # Errors
///
/// Returns [`EpochError::ForeignLog`] if a log's block hash does not match
/// `l1_hash`, or [`EpochError::DuplicateLogIndex`] if two logs share a
/// `log_index`.
pub fn derive_epoch(
    l1_number: u64,
    l1_hash: B256,
    ...
    let mut ordered: Vec<&LockboxLog> = logs
        .iter()
        .map(|log| {
            if log.block_hash() == l1_hash {
                Ok(log)
            } else {
                Err(EpochError::ForeignLog {
                    expected: l1_hash,
