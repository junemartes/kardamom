...
pub struct ExecutorFileConfig {
    /// Optional cluster-sealer client config. Default disabled (the Aeron
    /// tx_ordering subscription path is unchanged).
    pub cluster: ClusterConfig,
}

/// Cluster-sealer client config. Default disabled (IPC/MDC path unchanged).
