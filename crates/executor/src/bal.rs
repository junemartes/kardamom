...
/// outcome.
fn publish_with_retry(pubh: &PubHandle, bytes: &[u8], block: u64) -> PublishOutcome {
    let deadline = Instant::now() + PUBLISH_DEADLINE;
    loop {
        let ControlFlow::Break(result) = try_publish_or_wait(pubh, bytes, block, deadline) else {
            continue;
        };
        return result;
    }
...
/// once `try_publish` resolves; otherwise sleeps the retry backoff and
/// returns [`ControlFlow::Continue`], so the caller's loop tries again.
...
/// publish with retry, and count the block in the retention window.
/// Does nothing if `handoff` is `None` (an idle tick) or encoding fails.
///
/// The frame-bytes histogram below records a byte count that stays far
/// under 2^52 (the f64 mantissa).
#[allow(
    clippy::cast_precision_loss,
...
fn process_tick(
    ...
    // the retention window regardless of the live-delivery outcome: the
    // archive ring is the durability path (a validator that was down
    // catches up from it). See `publish_with_retry`'s doc comment for
    // the retry logic.
...
pub fn run_bal_publisher(rx: Receiver<BalHandoff>, pubh: PubHandle) {
    ...
    let measure: Vec<u16> = std::env::var("KARDAMOM_BAL_MEASURE")
        ...
        .unwrap_or_default();
    let mut retention = RetentionWindow::default();
    ...
    loop {
        ...
        process_tick(&pubh, granularity, &measure, &mut retention, handoff);
    }
