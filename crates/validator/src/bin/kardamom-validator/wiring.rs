...
impl Opened {
    ...
    pub(crate) fn open_streams(self) -> Result<Streamed> {
        ...
        // by default.
        let interop_serve = open_interop_serve(args, self.state.chain_id);
...
fn open_interop_serve(args: &Args, chain_id: u64) -> Option<InteropServe> {
    args.serve_feed.map(|addr| InteropServe {
        addr,
        store: Arc::new(kardamom_validator::interop::FeedStore::new(
            chain_id,
...
impl Ready {
    ...
    pub(crate) async fn run(self) -> Result<()> {
        ...
        // lifetime.
        let _feed_server = start_interop_serving(args, chain_id, interop_serve).await?;
