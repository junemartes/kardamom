...
//!
//! Each test brings up its own stack on OS-assigned ports, so the suite runs
...
//!
//! The suite is split across sibling files, pulled in with `include!` rather
//! than `mod` so every test keeps its exact pre-split name (`s7_…`, not
//! `consistency::s7_…`): CI filter strings and historical test names stay
//! valid, and `--test chain_semantics` still names this one target.
...
/// The scenario seam for a stack running the default 30 s ingress park bound.
/// Tests that tune the park bound derive their own `client_timeout(park)`.
...
/// Launch a stack whose config sets `l1: true`, skipping the test (early
/// return) when the `anvil` binary is absent — the convention every L1-backed
/// test here follows.
