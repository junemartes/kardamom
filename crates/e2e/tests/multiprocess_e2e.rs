...
/// Recorder/quorum durability round-trip (#36 + #38).
///
/// Brings up the full pipeline PLUS three `kardamom-recorder` processes
/// (recorder ids 0/1/2, recording the IPC `tx_ordering` stream) and one
/// `kardamom-recorder --aggregate` quorum process, and runs the ingress with
/// `--ack-policy on-quorum`. A submit only resolves once the quorum watermark
/// (Q=2 of N=3 recorders fsynced) has advanced past the tx's ordering
/// position — so a green receipt proves the recorder→watermark→aggregator→
/// ingress-gate path works end to end (exercises the #36 `--log-config`
/// plumbing on the recorders + the #38 quorum machinery).
///
...
async fn multiprocess_quorum_e2e_recorder_quorum_and_redundancy() {
    ...
    // ----- Recorders: one per id, recording tx_ordering. Kept in a Vec so we
    // ----- can drop (SIGTERM) one for the redundancy phase.
    let mut recorders: Vec<ChildGuard> = (0u8..3)
        .map(|rid| {
            ChildGuard::spawn(
                "kardamom-recorder",
                Command::new(target_bin.join("kardamom-recorder"))
                    .arg("--log-config")
                    .arg(&recorder_log_cfg)
                    .arg("--aeron-dir")
                    .arg(&aeron_dir)
                    .arg("--recorder-id")
                    .arg(rid.to_string())
    ...
    let ingress = ChildGuard::spawn(
        ...
        Command::new(target_bin.join("kardamom-ingress"))
            ...
            .arg("--jsonrpc-bind")
            .arg(format!("127.0.0.1:{JSONRPC_PORT}"))
            // The whole point: gate on the quorum watermark the aggregator
            // publishes. If the recorder/quorum path is broken, submits park
            // forever and the request below times out.
            .arg("--ack-policy")
            .arg("on-quorum")
            .stdout(Stdio::inherit())
            .stderr(Stdio::inherit()),
    ...
    wait_for_jsonrpc(&rpc_url, Duration::from_secs(30))
        .await
        .expect("ingress JSON-RPC ready");
    // Recorders + every subscription need a beat to attach before we publish
    // (Aeron has no pre-subscription replay); recorders also have to start
    // their recording before tx_ordering frames flow.
    ...
    // ----- Phase 1: with all 3 recorders, submits must ack under on-quorum.
    submit_and_assert_receipt(&client, &alice, bob_addr, 0).await;
    submit_and_assert_receipt(&client, &alice, bob_addr, 1).await;
    tracing::info!("on-quorum acks confirmed with 3/3 recorders");

    // ----- Phase 2: kill one recorder. Q=2 of N=3 still holds, so submits
    // ----- must continue to ack (redundancy).
    let killed = recorders.pop().expect("3 recorders");
    drop(killed); // SIGTERM
    tracing::info!("dropped one recorder; verifying quorum still holds at 2/3");
    tokio::time::sleep(Duration::from_secs(3)).await;
    submit_and_assert_receipt(&client, &alice, bob_addr, 2).await;
    submit_and_assert_receipt(&client, &alice, bob_addr, 3).await;
    tracing::info!("on-quorum acks still confirmed after losing 1 recorder");
...
/// Submit one signed transfer and assert the receipt comes back `status=0x1`.
/// With `--ack-policy on-quorum` the `eth_sendRawTransaction` call only
/// resolves once the quorum watermark has advanced past the tx.
async fn submit_and_assert_receipt(
    client: &jsonrpsee::http_client::HttpClient,
    signer: &PrivateKeySigner,
    to: Address,
    nonce: u64,
) {
    let raw = signed_transfer(signer, to, U256::from(1u64), nonce);
    let raw_hex = format!("0x{}", hex::encode(&raw));
    let returned_hash: B256 = client
        .request("eth_sendRawTransaction", rpc_params![&raw_hex])
        .await
        .unwrap_or_else(|e| panic!("tx {nonce}: eth_sendRawTransaction (quorum ack) failed: {e}"));
    let env = ConsensusEnvelope::decode(&mut raw.as_slice()).expect("decode tx");
    assert_eq!(
        returned_hash,
        *env.tx_hash(),
    ...
    let receipt: Value = client
        .request("eth_getTransactionReceipt", rpc_params![returned_hash])
        .await
        .unwrap_or_else(|e| panic!("tx {nonce}: eth_getTransactionReceipt failed: {e}"));
    let status = receipt
        .get("status")
        .and_then(|v| v.as_str())
        .unwrap_or_default();
...
fn build_quorum_bins() {
    ...
    assert!(
        st.success(),
        "failed to build kardamom service + recorder bins"
    );
}
