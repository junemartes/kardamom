...
pub(crate) fn feed_msg(
    ...
    #[allow(
        ...
        reason = "the low byte of origin_block only needs to vary the fixture hash; test \
                   callers pass small constants, so truncation is intentional"
    )]
    let origin_block_byte = origin_block as u8;
    OutboxMessage {
        origin_block_number: origin_block,
        origin_block_hash: B256::repeat_byte(origin_block_byte),
