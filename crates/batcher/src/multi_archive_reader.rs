...
//!    each into a `(BPosition -> TxEnvelope)` map. The offline path keeps
//!    the per-A index in RAM. This works for batch sizes that fit a few
//!    segment files.
...
use crate::archive_reader::{TxDataSegmentReader, TxOrderingSegmentReader, TypedRecord};
...
pub enum ResolvedRecord {
    ...
    /// travels in the DA payload of the block it leads.
    RemoteEpoch {
        position: BPosition,
...
///
/// Each archive is one segment file. The offline batcher reads whole epochs
/// at a time, so segment-roll handling belongs in the orchestration layer,
...
impl MultiArchiveConfig {
    ...
    ///
    /// # Errors
    /// Returns an error when an entry is missing its `=` separator, its
    /// sequencer id does not parse as `u8`, or a sequencer id repeats.
    pub fn parse_a_spec(spec: &str) -> Result<HashMap<u8, PathBuf>, BatcherError> {
        let mut out = HashMap::new();
        spec.split(',')
            .map(str::trim)
            .filter(|s| !s.is_empty())
            .try_for_each(|entry| Self::insert_a_spec_entry(&mut out, entry))?;
        Ok(out)
    }

    /// Parse one `sid=path` entry of a `--channel-a-archive` spec, and
    /// insert it into `out`.
    fn insert_a_spec_entry(
        out: &mut HashMap<u8, PathBuf>,
        entry: &str,
    ) -> Result<(), BatcherError> {
...
impl MultiArchiveReader {
    ...
    ///
    /// # Errors
    /// Returns an error when the B segment or any A segment cannot be
    /// opened or indexed.
    ...
    fn resolve(
        a_indexes: &HashMap<u8, PerASegmentIndex>,
        r: &TxRef,
    ) -> Result<TxEnvelope, BatcherError> {
        let idx = a_indexes.get(&r.shard_id).ok_or_else(|| {
            BatcherError::Config(format!(
    ...
    /// Turn one raw `tx_ordering` record into a `next()` result. `None`
    /// skips it: the batcher sends only user-tx data and block boundaries
    /// to L1. L1 deposits do not need to go back into a batch, and epochs
    /// are skipped by design — a reconstructor re-derives an epoch's
    /// deposits from L1 itself, using the block's `l1_origin`; putting
    /// deposits in the blob would waste space and add an unverifiable
    /// claim, because deposits are unsigned. `Some` yields the resolved
    /// record: a [`TxRef`] resolved against the A indexes, or a remote
    /// epoch or boundary marker carried as-is — remote messages are NOT
    /// re-derivable from this chain's L1 origin, so they DO travel in DA
    /// (spec §16 Q8), messages by value, for the accumulator to buffer
...
///
/// # Errors
/// Returns an error when the segment cannot be opened or a frame in it is
/// corrupt, or when two records share a `BPosition`.
pub fn load_a_index(path: &Path) -> Result<PerASegmentIndex, BatcherError> {
    let mut reader = TxDataSegmentReader::open(path)?;
    let mut idx = HashMap::new();
    reader.try_for_each(|rec| -> Result<(), BatcherError> {
        let rec = rec?;
