//! The per-table checks composing [`sweep`](super::sweep).
//!
//! Each check walks one table (or one coherent group, for the receipt
//! index), appends findings to the shared [`IntegrityReport`], and bumps the
...
///
/// Returns the decoded `last_committed_end_tx_position` cursor, which the
/// headers and receipts checks cross-reference.
