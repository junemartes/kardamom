...
//!
//! Loaded from TOML at process start, and passed to the supervisor and the
//! quorum aggregator. There is no global state.
//!
...
pub struct ChannelsConfig {
    ...
    /// and `tx_ordering_stream_id` remain because the surviving pieces
    /// still reference them: the archive recorder (durable-watermark path)
    /// and the offline archive reader record and replay tx_ordering frames
    /// on this `tx_ordering_stream_id`, and the in-crate
    /// `TxOrderingPublisher` and `TxOrderingSubscriber` (the unit-test
    /// fakes' real-Aeron counterparts) open this `tx_ordering_channel`.
    ...
    /// `base_port + 2*i` for receipts and `+ 2*i + 1` for boundaries. 0
    /// unless MDS is enabled.
    #[serde(default)]
    ...
    /// design ultimately wants. Today ingress simply attaches replicas
    /// `0..N` once at startup. This works because the executor job is a
    /// fixed `count` with `distinct_hosts`, so replica indices stay stable
    /// at `0..N`. A replica that restarts keeps its index and its
    /// endpoint, so the static attach stays correct across restarts.
    ///
    /// TODO(consul-watch): register each executor as a Consul service
    /// (`executor-receipts`) and have ingress watch it, calling
    /// `add_destination`/`remove_destination` on membership change instead
    /// of this static count. Until then this must match the executor job
    /// `count`. 0 (the default) is fine when MDS is disabled.
    #[serde(default)]
...
impl ChannelsConfig {
    ...
    ///
    /// MDS receipt fan-in computes per-replica ports as `base + 2*i (+ 1)`
    /// in unsigned arithmetic. A zero or negative base (the field stays
    /// `i32` for TOML-friendly signed parsing) would otherwise wrap
    /// silently into a nonsense endpoint port, and a base too close to
    /// 65535 would overflow the valid port range for the highest replica.
