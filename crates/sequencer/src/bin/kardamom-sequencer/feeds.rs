...
struct OriginPump<S, P> {
    shutdown: Shutdown,
    sub: S,
    publ: P,
}

impl<S, P> OriginPump<S, P> {
    ...
    fn run(
        mut self,
        mut step: impl FnMut(&mut S, &mut P) -> Result<bool, SequencerError>,
        ...
        loop {
            ...
            match step(&mut self.sub, &mut self.publ) {
                Ok(true) => idle.reset(),
