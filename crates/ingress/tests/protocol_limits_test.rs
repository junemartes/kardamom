...
//! gas limit above the EIP-7825 cap, or a blob (type-3) envelope — is refused
//! with a clear `-32602`-class error instead of burning into a
...
async fn gas_limit_above_the_eip7825_cap_is_rejected() {
    ...
    // 30M — a valid *block*'s worth of gas, but over the per-tx cap.
    ...
    // Exactly at the cap passes validation (and then parks awaiting a
    // receipt that the mock pipeline never produces — Timeout, not a
    // validation error).
