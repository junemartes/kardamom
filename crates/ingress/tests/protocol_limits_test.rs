//! Protocol-limit rejection at submission. A tx that can never execute,
...
use kardamom_ingress::test_support::{sign_eip4844, sign_legacy_with_gas};
...
async fn gas_limit_above_the_eip7825_cap_is_rejected() {
    ...
    let raw = sign_legacy_with_gas(&signer, 0, 30_000_000);
    ...
    let raw = sign_legacy_with_gas(&signer, 0, TX_GAS_LIMIT_CAP);
    let err = p.submit_raw(ip, raw).await.unwrap_err();
    assert!(
        !matches!(
            err,
