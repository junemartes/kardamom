...
//! split-architecture: M channel-A streams + 1 canonical channel-B).
//!
...
//! both the per-sequencer channel-A streams and the shared channel-B
...
/// enough Archive endpoints to host M per-sequencer channel-A streams
/// alongside the canonical channel-B stream. Today the single-node
...
async fn aeron_cluster_serves_m_plus_one_topology_smoke() {
    ...
    let cluster = AeronTestCluster::single_node()
        .await
        .expect("aeron container should start");
    // The harness exposes one shared Archive endpoint; all M channel-A
    // streams + the single channel-B stream live on it (per
    // `ChannelsConfig::tx_dattx_dattx_dattx_dattx_data_channel_template`'s `aeron:ipc?alias=a-{sid}`
